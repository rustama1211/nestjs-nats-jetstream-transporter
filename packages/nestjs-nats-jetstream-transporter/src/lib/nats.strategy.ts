...
export class NatsTransportStrategy extends Server implements CustomTransportStrategy {
  ...
  async listen(callback: typeof noop): Promise<void> {
    ...
    await this.subscribeToEventPatterns(this.jetstreamClient, this.jetstreamManager);
  ...
  async handleJetStreamMessage(message: JsMsg, handler: MessageHandler): Promise<void> {
    ...
    try {
      ...
      const signal = await handler(decoded, new NatsContext([message]))
        ...
        .then((observable: Observable<any>) => lastValueFrom(observable));
  ...
  async subscribeToEventPatterns(client: JetStreamClient, jsm: JetStreamManager): Promise<void> {
    ...
    for (const [pattern, handler] of eventHandlers) {
      ...
      const consumerConfig: Partial<ConsumerConfig> =  { name: `subscriber-${pattern}`, ack_policy: AckPolicy.Explicit, inactive_threshold: nanos(2* 60 * 1000), replay_policy: ReplayPolicy.Original };
      ...
      try {
        ...
        let consumerInfo: ConsumerInfo | null;
        try {
          consumerInfo = await jsm.consumers.info(pattern, defaultConsumerName);
        } catch (err) {
          consumerInfo = null;
        }
          if (!consumerInfo) {
            //create consumer based event pattern
            consumerInfo = await jsm.consumers.add(pattern, consumerConfig);
            //newConsumerInfo.
          } else {
            //update consumer absed event pattern
            // cannot update replay policy
            delete consumerConfig.replay_policy;
            consumerInfo = await jsm.consumers.update(pattern, defaultConsumerName, consumerConfig);
          }

        const eventConsumer = client.consumers.get(consumerInfo.stream_name, consumerInfo.name);
        (await eventConsumer).consume({callback: (message) => {
          if (message) {
        ...
        if ((error as Error).message  === "no stream matches subject") {
